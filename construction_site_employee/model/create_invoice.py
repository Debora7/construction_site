    ...
    def _createInvLines(self):
        ...
        if self.invoicing_type=='detailed':
            for task in self.task_ids:
                if task.task_employee_ids:
                    res += [(0, 0, {'name': _('Employee %s') % task.name, 'display_type':'line_section'})]
                    res += [
                    (0, 0, {
                        'name': " ".join([str(p.effective_hours), _("hours"), "-", task.name]),
                    ...
                    }) for p in task.task_employee_ids]
        return res
