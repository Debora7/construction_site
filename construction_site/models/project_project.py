...
class Project(models.Model):
    ...
    analytic_account_id = fields.Many2one("account.analytic.account", stored=True)
    ...
    def action_open_reports_balance(self):
        ...
        return {
            'name': 'Profit Reports',
            'type': 'ir.actions.act_window',
            'res_model': 'project.task.revenu.report',
    ...
    def action_open_decont(self):
        ...
        return {
            'name': 'Decont',
            'type': 'ir.actions.act_window',
            'res_model': 'project.site.invoice',
            'view_mode': 'list',
            'domain': [('task_id.project_id', '=', self.id)],
            'target': 'current',
        }
    ...
    def _destroy_any_managing_task(self):
        ...
        taskModel.search([('project_id','in',self.ids),]).write({
