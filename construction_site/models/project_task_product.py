...
class ProjectTaskProduct(models.Model):
    ...
    def _compute_requested_state(self):
        ...
        for s in self:
            ...
            if s.requested_qty < s.planned_qty:
                ...
                state = 'fix_request'
            elif s.requested_qty > s.planned_qty and s.planned_qty != 0:
                state = 'hiper_request'
    ...
    @api.depends(
        "stock_receive_ids",
        "stock_receive_ids.value",
    )
    def _compute_material_cost(self):
        for line in self:
            cost = 0
            for svl in line.stock_receive_ids:
                at_date = svl.picking_id.date
    ...
    def _compute_account_cost(self):
        for line in self:
            purchase_cost = 0
            for svl in line.stock_outer_ids:
                at_date = svl.move_id.date
