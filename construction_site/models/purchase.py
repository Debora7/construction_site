...
class PurchaseOrder(models.Model):
    ...
    task_id = fields.Many2one("project.task", stored=True)
...
class PurchaseOrderLine(models.Model):
    ...
    def _prepare_purchase_order_line_from_procurement(self, product_id, product_qty, product_uom, location_dest_id,
        ...
        if values.get('task_product_id', None):
            ...
            res['analytic_distribution'] = task_product.task_id.project_id._get_analytic_distribution()
        return res

    def _prepare_account_move_line(self, move=False):
        res = super(PurchaseOrderLine, self)._prepare_account_move_line(move)
        res.update({
            'project_id': self.order_id.task_id.project_id.id,
            'task_id': self.order_id.task_id.id,
            'analytic_distribution' : {
                self.order_id.task_id.project_id.analytic_account_id.id: 100
            }
        })
        return res
    ...
    def _distribute_qty(self, field_name):
        ...
        for task_prod in self.task_product_ids:
            convert_qty = task_prod.task_product_id.uom_id._compute_quantity(qty, self.product_uom_id)
            reserved_qty = convert_qty
            res.append((task_prod.id, reserved_qty))
            qty -= self.product_uom_id._compute_quantity(reserved_qty, task_prod.task_product_id.uom_id)
            if qty <= 0:
                break
        return res
