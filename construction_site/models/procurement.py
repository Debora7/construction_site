...
class MakeProcurement(models.Model):
    ...
    def _load_materials(self):
        ...
        if self.task_id:
            task_ids = self._colect_project_subtask(self.task_id)
            if not task_ids:
                subtree = self.task_id.search([('id', 'child_of', self.task_id.id)])
                task_ids = subtree.filtered(lambda t: t.need_procurement or any(
                    (tp.planned_qty - tp.requested_qty) > 0 for tp in t.task_product_ids
                ))
    ...
    def _incrementParentTask(self, tasks=None):
        # If caller provides an explicit `tasks` recordset, respect it
        # (even when empty) — this avoids accessing `self` after the
        # record(s) may have been unlinked. When `tasks` is None we
        # compute it from `self` as before.
        if tasks is None:
...
class MakeProcurementProduct(models.Model):
    ...
    def _getSupplier(self, value):
        ...
        try:
            ...
            _logger.info(f"Error on filter supplier {e}")
    ...
    def launch_replenishment(self, qty, replenishment, wh=None, route=None, location=None, planned_date=None):
        ...
        try:
            ...
            procurement = self.env['stock.reference'].Procurement(
                    self.product_id,
                    quantity,
                    uom_reference,
    ...
    def runProcurement(self, route=None, wh=None):
        ...
        replenishment = self.env['stock.reference'].create({
        ...
        self.env['stock.reference'].with_context(clean_context(self.env.context)).run(procurements)
