...
class Task(models.Model):
    ...
    product_ok = fields.Boolean("Produse")
    profitability_ok = fields.Boolean("Profitabilitate")
    ...
    def consumeProducts(self, force=False):
        for task in self:
            ...
            for pline in task.task_product_ids:
                ...
                pline.tmp_consume = 0
            self.env['stock.reference'].with_context(clean_context(self.env.context)).run(proc)
    ...
    def action_open_reports_material(self):
        ...
        action.update({'domain':[('task_id','=',self.id)]})
        return action
